...
import { useAppToast } from '../src/components/toast'
...
const mockCartItems: CartItem[] = [
  {
    id: '1',
    name: 'Manfinity Homme Men Letter Graphic Tee',
    image:
...
const CartScreen = () => {
  ...
  // 🛒 TODO: connect to your real cart store
  // const { items, updateQuantity, removeItem, clearCart } = useCartStore()
  const [cartItems, setCartItems] = useState<CartItem[]>(mockCartItems)
  const [selectedIds, setSelectedIds] = useState<string[]>(
    mockCartItems.map((i) => i.id)
  )
  ...
  const handleRemoveItem = (id: string) => {
    ...
    toast.show('Item removed from cart', { type: 'info' })
  }

  const selectedCount = selectedIds.length

  const { total, afterCoupon } = useMemo(() => {
    const selected = cartItems.filter((i) => selectedIds.includes(i.id))
    const total = selected.reduce(
      (sum, item) => sum + item.price * item.quantity,
      0
    )
    // Example coupon logic: 5€ off if total > 20
    const coupon = total >= 20 ? 5 : 0
    return { total, afterCoupon: Math.max(0, total - coupon) }
  ...
  const handleCheckout = () => {
    if (!selectedCount) {
      toast.show('Please select at least one item', { type: 'warning' })
      return
    }
    toast.show('Proceeding to checkout…', { type: 'success' })
          ...
          <View style={styles.fillSection}>
            ...
            >
              <View style={[styles.fillTab, styles.fillTabActive]}>
                <Text style={[styles.fillTabText, styles.fillTabTextActive]}>
                  All
                </Text>
              </View>
      ...
      <View style={styles.bottomBar}>
        ...
        <View style={styles.bottomCenter}>
          ...
          <Text style={styles.bottomAfterCouponText}>
            After coupon €{afterCoupon.toFixed(2)}
          </Text>
        </View>
